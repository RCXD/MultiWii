...
void GYRO_Common() {
	...
	if (calibratingG > 0) { //이 변수가 0보다 크면
		for (axis = 0; axis < 3; axis++) { //3개의 축에 대하여
			// Reset g[axis] at start of calibration
			if (calibratingG == 512) { //이 변수가 512라면
				g[axis] = 0; //자이로 관련 연산을 할 값인 g를 초기화

				#if defined(GYROCALIBRATIONFAILSAFE)
				previousGyroADC[axis] = imu.gyroADC[axis];
			}
			if (calibratingG % 10 == 0) { //이 변수가 512가 아니고, 10으로 나눠떨어진다면(예를 들면 1000이라던지)
		...
		#if defined(GYROCALIBRATIONFAILSAFE)
		if (tilt) { //위에서 연산이 끝나고서 abs(imu.gyroADC[axis] - previousGyroADC[axis]) > 8이 참이라면, tilt가 참이 된다.
			calibratingG = 1000; //이 연산을 하게되면 GYRO_Common()이 다시 실행될 때 
			LEDPIN_ON; //LED를 켜긴 하나 사용중이지 않음
		}
		else {
			calibratingG--;
			LEDPIN_OFF;
