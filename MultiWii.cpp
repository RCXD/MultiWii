...
// rc functions
// ******************
//아래는 rcSticks에 이용될 상수들 -> 01이면 Low, 11이면 Center, 10이면 Hich
#define ROL_LO  (1<<(2*ROLL))		//0b 0000 0001<<0 = 0b 0000 0001
#define ROL_CE  (3<<(2*ROLL))		//0b 0000 0011<<0 = 0b 0000 0011
#define ROL_HI  (2<<(2*ROLL))		//0b 0000 0010<<0 = 0b 0000 0010
#define PIT_LO  (1<<(2*PITCH))		//0b 0000 0001<<2 = 0b 0000 0100
#define PIT_CE  (3<<(2*PITCH))		//0b 0000 0011<<2 = 0b 0000 1100
#define PIT_HI  (2<<(2*PITCH))		//0b 0000 0010<<2 = 0b 0000 1000
#define YAW_LO  (1<<(2*YAW))		//0b 0000 0001<<4 = 0b 0001 0000
#define YAW_CE  (3<<(2*YAW))		//0b 0000 0011<<4 = 0b 0011 0000
#define YAW_HI  (2<<(2*YAW))		//0b 0000 0010<<4 = 0b 0010 0000
#define THR_LO  (1<<(2*THROTTLE))	//0b 0000 0001<<6 = 0b 0100 0000
#define THR_CE  (3<<(2*THROTTLE))	//0b 0000 0011<<6 = 0b 1100 0000
#define THR_HI  (2<<(2*THROTTLE))	//0b 0000 0010<<6 = 0b 1000 0000
...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
	...
	if (currentTime > GPSLEDTime) {           // - Fix and sat no. >= 5 -> LED blinks, one blink for 5 sat, two blinks for 6 sat, three for 7 ...
		if (f.GPS_FIX && GPS_numSat >= 5) {
			...
			if (blcnt >= 10 && ((blcnt % 2) == 0)) { STABLEPIN_ON; }
			else { STABLEPIN_OFF; }
		}
		else {
			if ((GPS_update == 1) && !f.GPS_FIX) { STABLEPIN_ON; }
			else { STABLEPIN_OFF; }
			blcnt = 0;
		}
	}
...
//////////////////////////
/* Hardware serial - println to terminal, designed by kch
unsigned char ind = 0;
unsigned char kch_str[] = "ardiono";
while (kch_str[ind] != '\0')
{
	SerialWrite(0, kch_str[ind++]);
}
*/
//////////////////////////

//send string to terminal function for debug, designed by kch
void SerialPrint(char *kch_str)
{
	unsigned char kch_ind = 0;
	while (kch_str[kch_ind] != '\0') SerialWrite(0, kch_str[kch_ind++]);
	//�꽕紐�: indicator(kch_ind)瑜� 0遺��꽣 1�뵫 利앷��떆�궎硫댁꽌 string�쓽 媛� 臾몄옄瑜� �븯�굹�뵫 �쟾�넚
	//�궗�슜 �삁: SerialPrint("arduino");
	//二쇱쓽�궗�빆: �떆由ъ뼹 紐⑤땲�꽣�뒗 �븘�몢�씠�끂 蹂대뱶�뿉�꽌 TX0, RX0�� 臾쇰젮 �엳�쑝誘�濡� 0踰� �떆由ъ뼹 而댄룷�듃瑜� �궗�슜
}
void SerialPrintln(char *kch_str)
{
	unsigned char kch_ind = 0;
	while (kch_str[kch_ind] != '\0') SerialWrite(0, kch_str[kch_ind++]);
	SerialWrite(0, '\n');
	//�꽕紐�: indicator(kch_ind)瑜� 0遺��꽣 1�뵫 利앷��떆�궎硫댁꽌 string�쓽 媛� 臾몄옄瑜� �븯�굹�뵫 �쟾�넚
	//�궗�슜 �삁: SerialPrintln("arduino");
	//二쇱쓽�궗�빆: �떆由ъ뼹 紐⑤땲�꽣�뒗 �븘�몢�씠�끂 蹂대뱶�뿉�꽌 TX0, RX0�� 臾쇰젮 �엳�쑝誘�濡� 0踰� �떆由ъ뼹 而댄룷�듃瑜� �궗�슜
}
...
void setup() {
	...
	#endif
	#if defined(MEGA)
	//SerialPrintln("Arduino Mega Drone setup");
	SerialOpen(1, SERIAL1_COM_SPEED); //UCSR1A Open
	SerialOpen(2, SERIAL2_COM_SPEED); //UCSR2A Open
	SerialOpen(3, SERIAL3_COM_SPEED); //UCSR3A Open
	#endif
	#endif									

	LEDPIN_PINMODE; //13, 30踰� ���쓣 異쒕젰�쑝濡�
	POWERPIN_PINMODE; //37踰덉쓣 異쒕젰�쑝濡�
	BUZZERPIN_PINMODE; //32踰덉쓣 異쒕젰�쑝濡�
	STABLEPIN_PINMODE; //31踰덉쓣 異쒕젰�쑝濡�
	POWERPIN_OFF; //PortC�쓽 0踰덈퉬�듃�쓽 異쒕젰�쓣 0�쑝濡�(�겢由ъ뼱)
	//SerialPrintln("Pinmode setting done");

	initOutput(); //紐⑤뱺 PWM���쓣 異쒕젰�쑝濡�(紐⑦꽣 �닔留뚰겮, QuadX�씠誘�濡� 4媛�), PWM�쓣 �쐞�븳 Timer interrupt �꽭�똿(Timer3 ,Timer4), 紐⑤뱺 紐⑦꽣�뿉 珥덇린 異쒕젰
	//SerialPrintln("initiating Output");
	readGlobalSet();
	//SerialPrintln("reading Global Set");
	...
	//SerialPrintln("Done reading program memory");
	#endif
	...
	//SerialPrintln("Done Reading EEPROM");

	#if GPS									
	recallGPSconf();								//Load GPS configuration parameteres
	#endif
	//SerialPrintln("Done recall GPS config");

	configureReceiver();
	//SerialPrintln("Done config Reciever");
	...
	#endif
	initSensors();
	//SerialPrintln("Done initiating Sensors");
	#if defined(I2C_GPS) || defined(GPS_SERIAL)
	GPS_set_pids();
	#endif
	previousTime = micros(); //loop() 함수의 time scheduling을 위한 변수인 듯? 확인 바람.
	#if defined(GIMBAL)
	calibratingA = 512;
	#endif
	calibratingG = 512;
	...
	GPS_SerialInit(); //SerialPrintln("Done initiating GPS Serial");
	...
	initLCD();
	//SerialPrintln("Done initiating LCD");
	#endif
	...
	//SerialPrintln("Done setup");
}


void go_arm() { //BOXARM
	//SerialPrintln("Go Arm");
	if (calibratingG == 0
...
void go_disarm() { //시동 해제 명령: f.ARMED 플래그를 clear
	//SerialPrintln("Go Disarm");
	if (f.ARMED) {
		f.ARMED = 0;
		#ifdef LOG_PERMANENT
...
void loop() {
	...
	static int16_t errorGyroI[2] = { 0,0 }; //ROLL, PITCH에 대한 Gyro Error값
	...
	/*
	unsigned char kch_input = '\0';
	while (SerialAvailable(0))
	{

		kch_input = SerialRead(0);
		SerialWrite(0, kch_input);
	...
	*/
	...
	if ((int16_t)(currentTime - rcTime) > 0) { // 50Hz
		...
		//SerialPrintln("Handling Sticks command");
		// checking sticks positions
			//Reference -> https://sites.google.com/a/eng.ucsd.edu/quadcopterclass/labs/lab-8-testing-programming-and-modifying-multiwii/part-5-the-controller
		uint8_t stTmp = 0; //rcData를 이용해 stick 데이터 처리를 위한 임시 변수 선언
		for (i = 0; i < 4; i++) {//rcData[]의 0번부터 3번 채널까지 검사(rcData의 0번~3번 채널은 모두 Stick 데이터)
								 //순서 참고: static uint8_t rcChannel[RC_CHANS] = { ROLLPIN, PITCHPIN, YAWPIN, THROTTLEPIN, AUX1PIN,AUX2PIN,AUX3PIN,AUX4PIN };
								 //ROLL, PITCH, YAW, THROTTLE 단자의 rcData 입력을 검사함.
			stTmp >>= 2; //stTmp값을 loop가 돌 때마다 4를 나눔(우측으로 2만큼 비트 shift)
			if (rcData[i] > MINCHECK) stTmp |= 0x80;      // check for MIN 
														  //rcData[]는 computeRC()를 통해 최종적으로 수신되고 필터링된 데이터
														  //이 값이 최저(MINCHECK)보다 크면 stTmp의 7번 비트를 set
			if (rcData[i] < MAXCHECK) stTmp |= 0x40;      // check for MAX
														  //다시 이 값이 최대(MAXCHECK)보다 작다면 stTmp의 6번 비트를 set
			//만약 위 if문을 모두 통과한다면(MINCHECK와 MAXCHECK 범위 내의 rc Data가 수신된다면)
			//stTmp는 최종적으로 0xff가 됨
			//알고리즘 분석 결론:
			//stTmp는 bit shift를 이용해서 어떤 rcData가 현재 min~max 범위 내에 있는지 아닌지를 알아내기 위한 변수
			//stTmp의 반환값을 0bAABBCCDD 꼴로 생각한다면 
			//AA~DD가 공통적으로 01이면 해당 핀이 MINCHECK를 넘어선 것이고 10이면 MAXCHECK를 넘어선 것이며 11이면 범위 내에 있는 것. 00이 나오려면 아주 빠른 찰나에 rcData가 바뀌어야함
			//이런 식으로 상위 2비트를 계산하고 2비트 밀어내는 식으로 stTmp 계산
			//rcSticks는 아래에서 다시 사용됨. Stick이 어떤 상태에 있는지 알아낼 수 있음.
		}
		if (stTmp == rcSticks) { //rcData를 이용해 인지한 stick data(stick의 상태)가 이전과 같은가?
			if (rcDelayCommand < 250) rcDelayCommand++; //rcDelayCommand는 아래에서 time sharing에 이용됨. stTmp = rcSticks인 상태, 
														//즉 stick데이터가 일정한 상태에서만 카운팅되고 변화가 인식되면 초기화
		}
		else rcDelayCommand = 0;
		rcSticks = stTmp; //스틱이 어떤 상태에 있는지 인식 -> 변화가 있다면 rcDelayCommand를 0으로.

		// perform actions    
		//SerialPrint("RC Data -> Throttle: "); 
		//SerialPrintln(int2char(rcData[THROTTLE])); //Throttle
		if (rcData[THROTTLE] <= MINCHECK) {            // THROTTLE at minimum //rcDelayCommand와는 무관하게, 스로틀이 MINCHECK보다 작다면 돌아감
													   //중요: THROTTLE이 최소값보다 작거나 같다고 인지되면, 아래에서 error값 초기화가 들어감
			#if !defined(FIXEDWING)
			errorGyroI[ROLL] = 0; errorGyroI[PITCH] = 0; //목표값과의 차이(정상상태 에러: steady state error)를 0으로 초기화시킴(ROLL, PITCH를 초기화시켜서 드론이 돌지 못하도록 하는 듯.)
			#if PID_CONTROLLER == 1
			errorGyroI_YAW = 0; //Yaw의 에러값 초기화(회전하지 못하도록)
			#elif PID_CONTROLLER == 2
			errorGyroI[YAW] = 0;
			#endif
			errorAngleI[ROLL] = 0; errorAngleI[PITCH] = 0; //Angle값에 대한 에러값 초기화
			#endif
						//Configuration(환경 설정)을 이용한 go_arm()/go_disarm() 호출
			if (conf.activate[BOXARM] > 0) {             // Arming/Disarming via ARM BOX
				if (rcOptions[BOXARM] && f.OK_TO_ARM) go_arm(); else if (f.ARMED) go_disarm(); //-> arm/disarm condition
			}
		}

		//rcDelayCommand == 20일 때의 일회성 동작
		if (rcDelayCommand == 20) { //rcDelayCommand 카운터 변수를 이용한 time scheduling -> ==인 이유는 단 한번만 실행되도록 하기 위함(루틴 실행에 에러가 전혀 없을 거라는 전제하에..)
			if (f.ARMED) {                   // actions during armed 
				#ifdef ALLOW_ARM_DISARM_VIA_TX_YAW
								//YAW를 이용한 disarm 조건
				if (conf.activate[BOXARM] == 0 && rcSticks == THR_LO + YAW_LO + PIT_CE + ROL_CE) go_disarm();    // Disarm via YAW : DISARM condition
				//XXX_LO는 MINCHECK보다 작은 rcData를, CE는 MINCHECK와 MAXCHECK  사이의 rcData를, HI는 MAXCHECK보다 큰 rcData를 인식했다는 것이다.
				//주의: THR_LO + YAW_CE + PIT_CE + ROL_CE 조합이 지상에서는 날거나 움직이지 않는 조건인데, YAW를 Low로 취해서 시동을 끄겠다는 얘기. 
				//공중에서 자유낙하하면서 YAW를 최대로 돌릴 일은 없으니, 지상이라면 시동을 끌 조건이란 얘기. ALLOW_ARM_DISARM_VIA_TX_ROLL을 이용하면 ROLL을 가지고 할 수도 있다.
				//conf.active[BOXARM]을 확인바람
				#endif
				#ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
				if (conf.activate[BOXARM] == 0 && rcSticks == THR_LO + YAW_CE + PIT_CE + ROL_LO) go_disarm();    // Disarm via ROLL
				#endif
			}
			else {                        // actions during not armed //주의: rcDelayCommand가 20이 찰 때 까지 stick을 유지해야 나타나는 일회성 동작
				i = 0;
				if (rcSticks == THR_LO + YAW_LO + PIT_LO + ROL_CE) {    // GYRO calibration
																		// 지상에서(THR_LO) YAW, PIT을 LO로 취해서 GYRO를 보정할 수 있도록 한 기능
					calibratingG = 512; //?
					#if GPS 
					GPS_reset_home_position(); //GYRO calibration과 동시에 GPS의 홈을 리셋하고 있다.
					#endif
					#if BARO //고도계를 이용하는 모드가 BARO 모드라고 알고는 있는데, 자세한 알고리즘 확인바람!!
					calibratingB = 10;  // calibrate baro to new ground level (10 * 25 ms = ~250 ms non blocking)
					#endif
					//IMU에서 calibrating을 확인할 필요가 있음!!
				}
				
				//INFLIGHT ACC CALIBRATION은 유용한 기능인거 같은데, 위험해서 사용 안하나? 정보 필요함.
				#if defined(INFLIGHT_ACC_CALIBRATION) 
				...
				if (rcSticks == THR_LO + YAW_HI + PIT_HI + ROL_CE) {            // Enter LCD config //LCD Config에 들어가려면 YAW와 PIT을 HI로 맞춰야 한다...
					#if defined(LCD_CONF)
					configurationLoop(); // beginning LCD configuration //LCD 환경설정 -> 추후 확인
					#endif
					previousTime = micros(); //이유는 모르겠으나, loop()가 다 끝나가지도 않았는데 LCD config 이후에 이 변수를 갱신하고 있다...??
				}

				#ifdef ALLOW_ARM_DISARM_VIA_TX_YAW
				else if (conf.activate[BOXARM] == 0 && rcSticks == THR_LO + YAW_HI + PIT_CE + ROL_CE) go_arm();      // Arm via YAW
																													 //ARM은 Calibration과는 다르다는 것이 확실함.
																													 //소프트웨어적으로 비행을 허가받으려면 go_arm()이 실행되어야 함.
																													 //stick을 IDLE상태에서 YAW만 HI로 당기면 시동이 걸림.(피드백은 없는 듯)
				#endif
				#ifdef ALLOW_ARM_DISARM_VIA_TX_ROLL
				...
				if (i) {
					writeParams(1); //parameter들을 Write하면서 LED를 Blink(인자로 1을 받으면 LED를 Blink함)
		...
		//AUX handling (50Hz)
		uint16_t auxState = 0;
		for (i = 0; i < 4; i++) //AUX1부터 AUX3까지 각 state가 LO, CE, HI인지 모두 확인
			auxState |= (rcData[AUX1 + i] < 1300) << (3 * i) | (1300 < rcData[AUX1 + i] && rcData[AUX1 + i] < 1700) << (3 * i + 1) | (rcData[AUX1 + i] > 1700) << (3 * i + 2);
		for (i = 0; i < CHECKBOXITEMS; i++) //CHECKBOXITEMS: 사용할 수 있는 기능 개수
			rcOptions[i] = (auxState & conf.activate[i]) > 0; //activate는 CHECKBOX의 활성 여부
		...
		if (rcOptions[BOXHEADFREE]) {
			...
			if ((f.GPS_FIX && GPS_numSat >= 5) && (GPS_distanceToHome > ADV_HEADFREE_RANGE)) {
				if (GPS_directionToHome < 180) { headFreeModeHold = GPS_directionToHome + 180; }
				else { headFreeModeHold = GPS_directionToHome - 180; }
		}
			#endif
		}
		else {
	...
	for (axis = 0; axis < 2; axis++) {
		...
		errorGyroI[axis] = constrain(errorGyroI[axis] + error, -16000, +16000);       // WindUp   16 bits is ok here
			//Refence: #define constrain(amt,low,high) ((amt)<(low)?(low):((amt)>(high)?(high):(amt)))
