...
static uint8_t rcChannel[RC_CHANS] = { ROLLPIN, PITCHPIN, YAWPIN, THROTTLEPIN, AUX1PIN,AUX2PIN,AUX3PIN,AUX4PIN }; //RC 최대 채널값을 저장하는 상수
...
//PCInt_RX_Pins[] 배열에, 2, 4, 5, 6, 7, 0, 1, 3 순서로 저장 -> 핀 비트를 선택하기 위한 상수
#endif
...
/***************                   RX Pin Setup                    ********************/
/**************************************************************************************/
void configureReceiver() { //수신기 환경설정
...
// PCINT activation
	for (uint8_t i = 0; i < PCINT_PIN_COUNT; i++) { // i think a for loop is ok for the init.
		...
		PCINT_RX_MASK |= PCInt_RX_Pins[i]; //ATmega2560의 PortK를 사용 중이므로 PCI2(Port Change Interrupt2)를 이용해야 한다. PCI1은 PE0와 PortJ, PCI0는 PortB이다.
		//ATmega2560에는 PCI2, PCI1, PCI0가 있다. PCINT는 INT는 같이 존재하고 구분된다. 흔히 쓰던 INT는 PD, PE에 있다. INT는 3종류의 트리거 모드가 있으나 PCINT는 toggle 트리거라고 보면 된다고 함.
		//PCINT는 Port 8개 중 하나만이라도 바뀌면 ISR에 들어간다고 하던데, 그래서 mask를 사용하는 듯.
	}
	PCICR = PCIR_PORT_BIT; //PCIE2 Enable (Mega2560상 A8~A15, 게임플러스에듀 드론실드 상 수신기 연결 단자)
...
/*************************   Special RX Setup   ********************************/
	//상단은 기본 PWM방식의 수신기 환경설정. PWM방식은 병렬로 신호를 수신한다.
	//PWM방식의 수신기 외적으로 Serial PPM_SUM, SBUS 등을 지원(Spectum은 어떤 방식인지는 찾아보지 않았으나 PPM SUM, SBUS가 직렬 제어 방식이므로 직렬 방식인 듯)
#endif
// Init PPM SUM RX
	...
	SerialOpen(1, 100000);
#endif
} //configureReceiver() done

/**************************************************************************************/
/***************               Standard RX Pins reading            ********************/ //RC핀 값을 읽는 부분
/**************************************************************************************/
#if defined(STANDARD_RX)
...
#define RX_PIN_CHECK(pin_pos, rc_value_pos)                          \
	...
	//dTime: Duty Time, cTime: Current Time, edgeTime: Edge 발생 시각
	//dTime이 900~2200us 범위에 들지 못하면 이전 edgeTime을 버리고 현재 시각을 cTime에 써넣는다.
//알고리즘: 
//mask에 해당 단자(pin_pos)가 걸리면 통과(mask에 걸리는 단자는 이전 서브루틴에서 변화가 감지되지 않았던 단자를 말함)
//현재 입력(pin)이 해당 단자(pin_pos)에서 일어나지 않았으면 dTime에 edgeTime으로부터 흐른 시간을 저장하고 통과
//dTime이 900~2200us조건에 들었다면 rcValue에 dTime 저장 -> dTime은 Duty가 된다. PWM방식이므로, Duty값인 dTime을 rcValue에 넣는다.
#endif




//추측: 아래 ISR은 PortK에 입력되는(하지만 나는 사용하지 않는) 수신 데이터(PWM)를 처리한다. 사용하지 않으려면 전체 주석처리해도 무관해보인다.
...
ISR(RX_PC_INTERRUPT) { //this ISR is common to every receiver channel, it is call everytime a change state occurs on a RX input pin
	...
	//현재 RX핀(Port K)의 입력 상태 확인

	mask = pin ^ PCintLast;   // doing a ^ between the current interruption and the last one indicates wich pin changed -> 어떤 핀이 상태 변화가 있었는지 확인하는 변수
	cTime = micros();         // micros() return a uint32_t, but it is not usefull to keep the whole bits => we keep only 16 bits
	sei();                    // re enable other interrupts at this point, the rest of this interrupt is not so time critical and can be interrupted safely
	PCintLast = pin;          // we memorize the current state of all PINs [D0-D7]
	//마스크: 입력 상태와 최근의 입력 상태를 XOR한 것 -> 즉, 인터럽트 되지 않은 핀인지 확인

	//rcValue[]를 다루는 코드 -> 마스크 조건을 통과하면 해당 RX 단자(PortK 중 해당하는 핀)의 rcValue를 계산한다.
#if (PCINT_PIN_COUNT > 0)
	RX_PIN_CHECK(0, 2); //0번 RX단자의 신호를 처리해 rcValue[2]에 저장 [RC단자의 순서(0, 1, 2, 3, ...)와 rcValue의 순서는 딱히 중요하지 않다. 결국 똑같은 걸로 선언되어 있음]
#endif
#if (PCINT_PIN_COUNT > 1)
	RX_PIN_CHECK(1, 4); //1번 RX단자의 신호를 처리해 rcValue[4]에 저장
#endif
...
#define SBUS_ENDBYTE 0x00
	...
	uint16_t readRawRC(uint8_t chan) { //rcValue를 읽어 반환. 단, 읽는 과정에만 모든 인터럽트 차단
		uint16_t data;
	...
	void computeRC() {
		...
		///rc4ValuesIndex++;
		///if (rc4ValuesIndex == 4) rc4ValuesIndex = 0; //4개의 Sample을 받기 위한 Index 처리
		for (chan = 0; chan < RC_CHANS; chan++) { //8개의 RC 채널에 대하여
#if defined(FAILSAFE)
...
#else
			///rcData4Values[chan][rc4ValuesIndex] = readRawRC(chan); //수신된 RC의 각 채널에 대한 raw 데이터를 하나씩 입력
#endif
#if defined(SPEKTRUM) || defined(SBUS) // no averaging for Spektrum & SBUS signal
			rcData[chan] = rcData4Values[chan][rc4ValuesIndex];
#else
			///rcDataMean[chan] = 0; //4차 이동평균필터를 적용하기 위한 변수 초기화
			///for (a = 0; a < 4; a++) ///rcDataMean[chan] += rcData4Values[chan][a]; //평균값에 지금껏 수신한 raw data 4개를 모두 더함
			///rcDataMean[chan] = (rcDataMean[chan] + 2) >> 2; //누적합에 2를 더해서 4로 나눔(왜 근소한 2를 더하는가? 쌓이는 값들은 900~2200인데)
															//이동평균필터: y[n] = (x[n] + x[n-1] + x[n-2] + x[n-3] - 2)/4
			///if (rcDataMean[chan] < (uint16_t)rcData[chan] - 3)  rcData[chan] = rcDataMean[chan] + 2;
			///if (rcDataMean[chan] > (uint16_t)rcData[chan] + 3)  rcData[chan] = rcDataMean[chan] - 2;
#endif		//Mean 값의 변화를 억누르는 필터 -> 3 이상 벌어지면 변화량을 2만큼 억누른다. ex) Mean이 1502->1508이면 최종적으로 1506 반환
