...
void computeIMU() { //IMU 데이터 분석 함수 -> loop문에서 실행
	uint8_t axis;
	...
	for (axis = 0; axis < 3; axis++) {
		...
		//y = x * (c - 1)+
		gyroSmooth[axis] = imu.gyroData[axis]; //gyroSmooth[] 갱신
	}
	#elif defined(TRI)
	...
	// 4) Optimization tricks: http://www.hackersdelight.org/ //최적화 트릭에 대한 자료가 있다! 아래 코드에 해당하는 내용이 있을 듯...
	//
...
#if BARO
uint8_t getEstimatedAltitude() { //Altitude 추정
	int32_t  BaroAlt;
	...
	alt.EstAlt = (alt.EstAlt * 6 + BaroAlt) >> 3; // additional LPF to reduce baro noise (faster by 30 쨉s)
